...
public class Chess implements Runnable {
    ...
    public void draw(Graphics g) {
        ...
        if(check) {
            ...
            g.drawString("CHECK U GOOBER", TILE_SIZE*(WIDTH_IN_TILES/2), TILE_SIZE*(WIDTH_IN_TILES/2));
        }
    }
        ...
        for(int j = 0; j < pieces[i].length; j++) {
            ...
            switch(p.getPieceType()) {
                ...
                if(i + direction < 0 || i + direction >= pieces.length) {
                    ...
                    if(j + 1 < pieces.length && pieces[i + direction][j+1] != null && pieces[i + direction][j+1].getPieceColour() != p.getPieceColour()) {
                        validMoves[i + direction][j+1] = true;
                    }
                    if(j - 1 >= 0 && pieces[i + direction][j-1] != null && pieces[i + direction][j-1].getPieceColour() != p.getPieceColour()) {
                        validMoves[i + direction][j-1] = true;
                    }
                break;
